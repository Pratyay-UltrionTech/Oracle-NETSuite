...
from app.services.netsuite_service import fetch_items_from_netsuite
...
def _matches_search(row: Dict[str, str], search: str) -> bool:
    if not search:
        return True
    s = search.lower()
    for key in ("displayName", "internalId", "itemCategory", "hsnCode"):
...
def _to_api_row(row: Dict[str, str]) -> Dict[str, Any]:
    ...
    return {
        ...
        "itemCategory": str(row.get("itemCategory") or ""),
        "department": str(row.get("department") or ""),
        "className": str(row.get("className") or ""),
        "location": str(row.get("location") or ""),
        "hsnCode": str(row.get("hsnCode") or ""),
        "gstRate": str(row.get("gstRate") or ""),
        "source": "netsuite",
        "isActive": True,
    }


async def fetch_items_live(*, force_refresh: bool = False) -> List[Dict[str, str]]:
    now = time.time()
    ...
    rows = await fetch_items_from_netsuite()
    _cache["rows"] = rows
    _cache["at"] = now
    logger.info("Item cache refreshed from NetSuite count=%s", len(rows))
    return rows
...
async def _log_fetch(
    ...
    *,
    ...
    try:
        ...
        logger.exception("Failed to write FETCH_ITEMS activity log")


async def refresh_items_from_netsuite() -> Dict[str, Any]:
    rows = await fetch_items_live(force_refresh=True)
    return {
        "success": True,
        "fetched": len(rows),
        "cached": True,
...
async def get_items_live(
    ...
    *,
    ...
    try:
        rows = _filter_rows(await fetch_items_live(), search=search)
        page = max(1, page)
        ...
        logger.exception("Item live fetch failed")
...
async def search_items(
    ...
    *,
    ...
    rows = _filter_rows(await fetch_items_live(), search=q)
    total = len(rows)
    start = (page - 1) * limit
    page_rows = rows[start : start + limit]
    return {
...
async def get_item_by_internal_id(
    ...
    if not iid:
        return None
    for row in await fetch_items_live():
